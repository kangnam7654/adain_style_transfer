...
def main(args):
    ...
    content_input = image_loader(
        content_path, transform=transform, device=device
    )  # [1, C, H, W] torch.tensor
    style_input = image_loader(
        style_path, transform=transform, device=device
    )  # [1, C, H, W] torch.tensor
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        default=ROOT_DIR.joinpath("config", "config.yaml"),
        help="config의 경로입니다.",
    )
